/* Program 2. For DHT11 sensor and with web page where you can change settings.
...
void handleRoot() {
  ...
  <html lang=\"en\">\
    ...
    <body>\
            ...
            Target Temperature: <input type='text' id='setTargetTemp' value=           size=3 autofocus>\
            ...
            <br><button id=\"send_button\">Save</button>\
            <div>\
            </div>\
  ...
  for (int i=0; i<myTempStr.length(); i++){
    htmlResponse[319+i] = myTempStr[i];
  }
  for (int i=0; i<deviationStr.length(); i++){
    htmlResponse[413+i] = deviationStr[i];
  }
  for (int i=0; i<delayTimeStr.length(); i++){
    htmlResponse[504+i] = delayTimeStr[i];
  }
